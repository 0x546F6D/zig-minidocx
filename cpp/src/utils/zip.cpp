...
static size_t readStream(void *pOpaque, size_t file_ofs, void *pBuf,
                         size_t n) noexcept {
...
static size_t writeStream(void *pOpaque, size_t file_ofs, const void *pBuf,
                          size_t n) noexcept {
...
Zip::Zip() : zip_{new mz_zip_archive_ex}, file_{new std::fstream} {
...
Zip::Zip(Zip &&src) noexcept { swap(src); }
...
void Zip::open(const std::string &filename, const OpenMode openMode) {
  ...
  case OpenMode::ReadOnly:
    ...
    if (!mz_zip_reader_init(zip_, size, flags)) {
      file_->close();
      throw Exception(mz_zip_get_error_string(mz_zip_get_last_error(zip_)),
                      "miniz");
    }
...
static void iosmove(std::iostream *ios, const std::streampos dst,
                    const std::streampos src, const std::streamsize len) {
...
void Zip::deleteFiles(const std::vector<fs::path> &names) {
  ...
  for (uint32_t i{0}; i < numEntries; i++) {
    ...
    const auto it =
        std::find_if(selectedNames.begin(), selectedNames.end(),
                     [entryName](const std::string &selectedName) {
                       return entryName.starts_with(selectedName) &&
                              (entryName.size() == selectedName.size() ||
                               entryName[selectedName.size()] == '/');
                     });
  ...
  for (uint32_t i{0}; i < maxIndex; i++) {
    entryPos[indices[i]].lf_len_ =
        entryPos[indices[i + 1]].lf_ofs_ - entryPos[indices[i]].lf_ofs_;
    entryPos[i].cd_len_ = entryPos[i + 1].cd_ofs_ - entryPos[i].cd_ofs_;
  }
  entryPos[indices[maxIndex]].lf_len_ =
      zip_->m_archive_size - entryPos[indices[maxIndex]].lf_ofs_;
  ...
  do {
    while (index < numEntries &&
           entryCmd[indices[index]] == EntryCommand::Delete)
      rdPos += entryPos[indices[index++]].lf_len_;
    delLen = rdPos - wrPos;

    while (index < numEntries &&
           entryCmd[indices[index]] == EntryCommand::Move) {
      movLen += entryPos[indices[index]].lf_len_;
      entryPos[indices[index]].lf_ofs_ -= delLen;

      MZ_WRITE_LE32(&MZ_ZIP_ARRAY_ELEMENT(cdirPtr, mz_uint8,
                                          entryPos[indices[index]].cd_ofs_) +
  ...
  do {
    ...
    while (index < numEntries && entryCmd[index] != EntryCommand::Delete) {
      ...
      MZ_ZIP_ARRAY_ELEMENT(cdirOfsPtr, mz_uint32, cdirOfsPtr->m_size++) =
          entryPos[index].cd_ofs_;
    ...
    if (delLen) {
      std::memmove(&MZ_ZIP_ARRAY_ELEMENT(cdirPtr, mz_uint8, wrPos),
                   &MZ_ZIP_ARRAY_ELEMENT(cdirPtr, mz_uint8, rdPos), movLen);
    }
    wrPos += movLen;
    rdPos += movLen;
    movLen = 0;
...
bool Zip::hasEntry(const fs::path &name) {
  ...
  return mz_zip_reader_locate_file_v2(zip_, entryName.generic_string().c_str(),
                                      0, 0, &entryIndex);
}
...
size_t Zip::entrySize(const fs::path &name) {
  ...
  mz_uint32 entryIndex;
  if (!mz_zip_reader_locate_file_v2(zip_, entryName.generic_string().c_str(), 0,
                                    0, &entryIndex))
    throw Exception(mz_zip_get_error_string(mz_zip_get_last_error(zip_)),
                    "miniz");
...
Zip::extractFileToStream(const fs::path &name, std::ostream &dst) {
  ...
  mz_uint32 fileIndex;
  if (!mz_zip_reader_locate_file_v2(zip_, filename.generic_string().c_str(), 0,
                                    0, &fileIndex))
    throw Exception(mz_zip_get_error_string(mz_zip_get_last_error(zip_)),
                    "miniz");
...
#else
  fs::last_write_time(dst,
                      std::chrono::clock_cast<std::chrono::file_clock>(mtime));
#endif
}
...
void Zip::addFileFromMem(
    const fs::path &name, const void *buf, const size_t bufsize,
    const std::chrono::system_clock::time_point &modifiedTime) {
  if (openMode_ != OpenMode::Create && openMode_ != OpenMode::Create64 &&
      openMode_ != OpenMode::Update)
    throw invalid_operation();
  ...
  if (!mz_zip_writer_add_mem_ex_v2(zip_, filename.generic_string().data(), buf,
                                   bufsize, NULL, 0, MZ_DEFAULT_COMPRESSION, 0,
                                   0, &mtime, NULL, 0, NULL, 0))
    throw Exception(mz_zip_get_error_string(mz_zip_get_last_error(zip_)),
                    "miniz");
}

void Zip::addFileFromStream(
    const fs::path &name, std::istream &src,
    const std::chrono::system_clock::time_point &modifiedTime) {
  if (openMode_ != OpenMode::Create && openMode_ != OpenMode::Create64 &&
      openMode_ != OpenMode::Update)
    throw invalid_operation();
  ...
  if (!mz_zip_writer_add_read_buf_callback(
          zip_, filename.generic_string().data(), readStream, &src, maxSize,
          &mtime, NULL, 0, MZ_DEFAULT_COMPRESSION, NULL, 0, NULL, 0))
    throw Exception(mz_zip_get_error_string(mz_zip_get_last_error(zip_)),
                    "miniz");
}
...
void Zip::addFolder(const fs::path &name) {
  ...
  if (!mz_zip_writer_add_mem(zip_, dirname.generic_string().data(), NULL, 0,
